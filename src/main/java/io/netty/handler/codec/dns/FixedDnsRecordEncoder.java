...
public class FixedDnsRecordEncoder implements DnsRecordEncoder {
  ...
  private void encodePtrRecord(DnsPtrRecord record, ByteBuf out) throws Exception {
    ...
    encodeName(record.hostname(), buffer);
  ...
  private void encodeOptEcsRecord(DnsOptEcsRecord record, ByteBuf out) throws Exception {
    ...
    if (addressBits < sourcePrefixLength || sourcePrefixLength < 0) {
      throw new IllegalArgumentException(
          sourcePrefixLength + ": " + sourcePrefixLength + " (expected: 0 >= " + addressBits + ')');
    }
    ...
    final short addressNumber =
        (short)
            (bytes.length == 4
                ? InternetProtocolFamily.IPv4.addressNumber()
                : InternetProtocolFamily.IPv6.addressNumber());
